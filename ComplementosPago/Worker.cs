    ...
    public class Worker : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                {
                    var db = scope.ServiceProvider.GetRequiredService<ComplementoDbContext>();

                    var ejecuciones = await db.TiemposEjecucion
                        .Where(t => t.Activo && t.Dia.ToLower() == dia && t.Hora == horaMinuto)
                        .ToListAsync(stoppingToken);

                    if (ejecuciones.Any())
                    {
                        _logger.LogInformation("Ejecutando tarea programada para {dia} a las {hora}", dia, horaMinuto);
                        await LeerComplementosAsync(db);
                    }
                }
